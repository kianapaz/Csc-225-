...
public class HuffEncoder{
    ...
    public void traverse(TreeNode n, Stack<Integer> s, HuffFileWriter x){
        ...
        if(n.right == null && n.left == null){
            ...
            int[] symbolBits = reverse(s);

            // Create a byte[] with all the byte symbols
            byte symbol[] = {n.ch};
            // Create a temp Integer[] to transfer all the values from int[] to Integer[]
            Integer[] temp = new Integer[symbolBits.length];
            int i = 0;
            for(int item : symbolBits){
                temp[i++] = Integer.valueOf(item);
            }

            System.out.print(symbol[0]+";");
            for(int j = 0; j < symbolBits.length; j++){
                System.out.print(symbolBits[j]);
            }
            System.out.println();
            ...
            HuffFileSymbol encode = new HuffFileSymbol(symbol, symbolBits);
    ...
    public PriorityQueue<TreeNode> buildTree(LinkedList<Byte> input_bytes){
        ...
        while (itr.hasNext()){
            ...
            System.out.println(node.ch + "AAA" + node.frequency); 
        }
        System.out.println(); 
        ...
        while (pq.size() > 1) {
            ...
            System.out.println(node1.ch + "*" + node1.frequency); 
            System.out.println(node2.ch + "$" + node2.frequency); 
